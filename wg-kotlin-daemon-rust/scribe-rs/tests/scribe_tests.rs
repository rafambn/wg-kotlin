...
async fn new_scroll_contains_scroll_id() {
    ...
    let id = scroll
        ...
        .expect("scroll_id should exist and be string");
    assert!(!id.is_empty(), "scroll_id should not be empty");
}

#[tokio::test]
async fn new_scroll_inherits_imprint_values() {
    let scribe = Scribe::builder()
        .imprint("service", "wg-daemon")
        .imprint("version", "0.1.0")
        .build();

    let scroll = scribe.new_scroll(None);
    assert_eq!(
        scroll.get("service").and_then(Value::as_str),
        Some("wg-daemon")
    );
...
async fn saver_receives_sealed_scrolls() {
    let memory = Arc::new(MemorySaver {
    ...
    assert_eq!(
        sealed.data.get("scroll_id").and_then(Value::as_str),
        Some("scroll-1")
    );
    assert_eq!(
        sealed.data.get("event").and_then(Value::as_str),
        Some("daemon_startup")
    );
}
...
async fn sealed_scroll_matches_expected_wire_shape() {
    ...
    scroll.insert(
        "event".to_string(),
        Value::String("daemon_check".to_string()),
    );
    ...
    assert_eq!(
        object.len(),
        2,
        "sealed scroll top-level should have success + data only"
    );
    assert_eq!(json["success"], Value::Bool(false));
    assert_eq!(
        json["data"]["scroll_id"],
        Value::String("shape-1".to_string())
    );
    assert_eq!(
        json["data"]["service"],
        Value::String("wg-daemon".to_string())
    );
    assert_eq!(
        json["data"]["event"],
        Value::String("daemon_check".to_string())
